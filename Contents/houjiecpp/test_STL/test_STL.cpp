...
namespace jj01  //test_array
{
    void test_array()
    {
        ...
        clock_t timeStart = clock();
        for(long i=0; i< ASIZE; ++i) {   //ASIZE 500000
            c[i] = rand();              //随机数
        }
        cout << "milli-seconds : " << (clock()-timeStart) << endl;	// 看一下500000次循环消耗的时间
        cout << "array.size()= " << c.size() << endl;
        cout << "array.front()= " << c.front() << endl;  // 第一个数（随机的，没有实际的意义）
        cout << "array.back()= " << c.back() << endl;    //最后一个数 （随机的，没有实际的意义）
        cout << "array.data()= " << c.data() << endl;  //该数组起点的地址
        ...
        cout << "qsort()+bsearch(), milli-seconds : " << (clock()-timeStart) << endl;	// 排序加查找消耗的时间
        if (pItem != NULL)
...
namespace jj02 //test_vector
{
    void test_vector(long& value)
    {
        ...
        {
            try {
                snprintf(buf, 10, "%d", rand());  //随机数转为string
                c.push_back(string(buf));
            }
            catch(exception& p) {
                cout << "i=" << i << " " << p.what() << endl;
                //曾經最高 i=58389486 then std::bad_alloc
                abort(); // 如果有异常发生，如value太大要不到内存，则退出程序
            }
        }
        ...
        {
            timeStart = clock();
            auto pItem = ::find(c.begin(), c.end(), target);  //查找，返回iterator, ::表示全局
...
#include <iostream>
#include <ctime>
namespace jj04 //test_forward_list
{
...
namespace jj07 //test_multimap
{
    void test_multimap(long& value)
    {
        ...
        cout << "c.find(), milli-seconds : " << (clock()-timeStart) << endl;
        if (pItem != c.end())
            cout << "found, value=" << (*pItem).second << endl; //第一个是key,第二个是value,所以用.second
        else
            cout << "not found! " << endl;
...
//---------------------------------------------------
    ...
    * 测试结果：
    ...
    array.back()= 29794
    array.data()= 0x6a7950
    target (0~32767):32000    此为手动输入的值
     qsort()+bsearch(), milli-seconds : 74
    found, 32000
    *
    * */
...
//---------------------------------------------------
   // long value = 1000000 ;
    ...
    construction, milli-seconds : 206
    size()= 1000000
    8MyString --
    CCtor=0 MCtor=2048575 CAsgn=0 MAsgn=0 Dtor=2048575 Ctor=1000000 DCtor=0
    copy, milli-seconds : 125
    move copy, milli-seconds : 1
    swap, milli-seconds : 0


    test, with non-moveable elements
    construction, milli-seconds : 502
    size()= 1000000
    11MyStrNoMove --
    CCtor=2048575 MCtor=0 CAsgn=0 MAsgn=0 Dtor=2048575 Ctor=1000000 DCtor=0
    copy, milli-seconds : 125
    move copy, milli-seconds : 1
    swap, milli-seconds : 0
     *
